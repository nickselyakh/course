...
$$ 
\begin{aligned}
        & \min_{u(\cdot | t)} \sum_{t}^{t+N-1} L(x(k(t), u(k(t))) + F(x(t+N)t)), \\ 
        ...
        & u^{*}(\cdot \ | \ t) \Rightarrow x := [x^T(t+1 \ |\ t), x^T(t+N \ |\ t)]^T \\
        & u := [u^T(t \ |\ t), u^T(t+N-1 \ |\ t)]^T \\
    \end{aligned}
$$
\begin{equation} \label{ch4-1}
    J(x(t), u(\cdot \ | \ t)) = x^T(t\ |\ Qx(t) + x^T\tilde{Q}x + u^T\tilde{R}u)
\end{equation}
$$x(t+k \ |\ t) = A^kx(t) + \sum_{j=0}^{k-1}A^jBu(t+k-j-1 \ |\ t)$$
...
$$
    J(x(t), u) = 1/2 x^T(t)Yx(t) + 1/2 u^THu + x^T(t)FU_x.
$$
...
\subsection{Успокоение материальной точки}
    $$ 
    \begin{cases}
    & x_f \to \min, \\
    & \dot{x} = Ax + Bu, \ x(0) = x_{0},\ x(t_f) = x_f, \\
    & \|u\|_{\infty} \le 1, \ t \in T = [0,\ t_f]. \\
    \end{cases}
    $$

Управление принимает лишь значения $\pm1$. Задача синтеза будет решена, если в пространстве $\mathbb{R}^n$ мы укажем области $s_{-}$ и $s_{+}$ такие что, когда \\
$x \in s_{-} \Rightarrow u(t) = -1$ \\
$x \in s_{+} \Rightarrow u(t) = 1$ \\
$s_{-} \cup s_{+} = \mathbb{R}^n$\\
Границы этих областей --- поверхности переключения. Рассмотрим задачу о наискорейшем успокоении с помощью граничных управлений материальной точки единичной массы движущейся по прямой из заданного начального положения $x(0) = x_{01}$ с заданной начальной скоростью $\dot{x}(0) = x_{02}$. Под наискорейшим успокоением понимается перевод в начало координат и остановка за минимальное время $t_f$.\\
$x(t_f) = 0$
$\dot{x}(t_f) = 0$ \\
$$
t_f \to \min \\
...
$$
$$
x_{1}(0) = x_{01} \ \ \ x_{1}(t_f) = 0 \ \ \ x = x_{1}
$$
$$
x_{2}(0) = x_{02} \ \ \ x_{2}(t_f) = 0 \ \ \ \dot{x} = x_{2}
$$
$$|u(t)| \le 1, \ t \in [0,\ t_f]$$
$$A = \begin{pmatrix}
    0 & 1\\
...
\end{pmatrix} - b = \begin{pmatrix}
    ...
    1\\
\end{pmatrix}$$
$$x \in R^2, \ \dot{x} = Ax + Bu$$
$$H(x, \psi, u) = \psi^{'}(Ax + Bu) = \psi_{1}x_{2} +  \psi_{2}u$$
$$
\begin{cases}
    \dot{\psi_{1}} = 0\\
    \dot{\psi_{2}} = -\psi_{1}
...
$$
$$
\psi_{1}(t) = C_{1},\ \psi_{2}(t) = -C_{1}t + C_{2}, \ u(t) = sign(\psi_{2}(t)), \ t \ge 0
$$

$\psi_{2}(t)$ линейна $\Rightarrow$ меняет знак не более одного раза \\

\begin{enumerate}
    \item
        $$
        u = 1 \Rightarrow \begin{cases}
            \dot{x_{1}} = x_{2}\\
            \dot{x_{2}} = 1\\
        \end{cases}
        $$\\
        ...
        $$
        $$
        x_{1} = t^{2}/2 + C_{1}t + C_{2}
        $$
        $$
        t = x_{2} - C_{1}
        $$
        $$
        x_{1} = x_{2}^{2}/2 + \overline{C}
        $$
