...
\begin{equation*}
    x(t_f) = F(t_f, t_0)x_0 + \int_{t_0}^{t_f}F(t_f, \theta)b(\theta)u(\theta)d\theta.
\end{equation*}
...
\begin{equation*}
    \begin{aligned}
        & x(t_f) = F(t_f, t_0)x_0 + \sum_{s=t_0}^{t_f-h} \int_{s}^{s+h}F(t_f, \theta)b(\theta)u(\theta)d\theta \\
        & = F(t_f, t_0)x_0 + \sum_{s \in T_h}\int_{s}^{s+h}F(t_f, \theta)b(\theta)d\theta u(s).
    \end{aligned}
\end{equation*}
...
\begin{equation*}
    \begin{aligned}
        & c^{'}x(t_f) = c^{'}F(t_f, t_0)x_0 + \sum_{s \in T_h}\int_{s}^{s+h} c^{'}F(t_f, \theta)b(\theta)d\theta u(s) \\
        = c^{'}F(t_f, t_0)x_0 +  \sum_{s \in T_h}c_h(s)u(s),
    \end{aligned}
\end{equation*}
где $c_h(s) = \int_{s}^{s+h} c^{'}F(t_f, \theta)b(\theta)d\theta$.

\begin{equation*}
    Hx(t_f) = HF(t_f, t_0)x_0 +  \sum_{s \in T_h}\int_{s}^{s+h} HF(t_f, \theta)b(\theta)d\theta u(s),
\end{equation*}
Произведем замену
\begin{equation*}
    d_h(s) = \int_{s}^{s+h} HF(t_f, \theta)b(\theta)d\theta,
\end{equation*}
и примем
\begin{equation*}
    Hx(t_f) = g,
\end{equation*}
получим следующее соотношений для $g(t_0)$:
\begin{equation*}
    g(t_0) = g - HF(t_f, t_0)x_0.
\end{equation*}
